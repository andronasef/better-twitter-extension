...
**Success Criteria** (what must be TRUE):
  1. User can load the extension, open the popup on x.com, and see a grid of category tiles that open into per-category panels, rendered in X's Chirp font, matching their active X theme (Light / Dim / Lights Out), with a tooltip explaining every toggle.
