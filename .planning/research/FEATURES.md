...
| Personal-use emoji reactions that render locally without posting | X shipped native emoji-style reactions on tweets starting ~April 2024, but as of the most recent reporting found, broad rollout beyond limited markets (e.g., Turkey) had not reached general availability by 2026 — the feature gap this project targets is real and still open. A third-party "Emoji reactions in Twitter" extension already exists, but it renders reactions **client-side only, visible to other users of the same extension** — it does not post anything to X. This is the materially safer competing pattern | MEDIUM (client-side-only) vs. HIGH (posting a real reply, as currently planned) | This is the single highest-risk feature in the project's plan (see Anti-Features and Pitfalls below) precisely because it diverges from how the one existing competitor implements the same idea. It is still a legitimate differentiator — a real, visible-to-everyone emoji reply is a stronger, more "native-feeling" interaction than a for-your-eyes-only overlay — but it inherits X's automation-detection risk in a way the client-side-only pattern does not. |
...
|---------|---------------|------------------|-------------|
| Bulk/automated liking, following, or replying beyond a single deliberate emoji reaction | Feels like a natural extension of "make X better" — "just auto-like everything from my list" | X explicitly analyzes interaction graphs, posting cadence, and account metadata for automation; users report **permanent suspensions with failed appeals** from exactly this category of extension/script. This is the most severe possible consequence in the whole feature space | Keep automation surface to the single, user-initiated, rate-naturally-limited emoji-reaction action already planned, with the 5s undo — never queue, schedule, or bulk-trigger actions |
...
[Emoji reaction posting] ──conflicts (risk-wise)──> [Chrome Web Store automated-posting scrutiny]
...
- **Emoji reaction posting conflicts, in risk terms, with Chrome Web Store scrutiny and X's automation detection:** this is a design constraint, not a blocking dependency — it means the feature must stay strictly single-action/user-initiated (no batching, no scheduling, no re-posting on undo-then-redo) to keep the risk bounded to what the project's Key Decisions already accept knowingly.
...
- [ ] Emoji reaction menu with posting + 5s undo — isolate as its own phase given it's the highest-risk feature; ship it last so the rest of the extension has already proven the DOM-injection and settings infrastructure it reuses
...
| Emoji reaction (posting, with undo) | MEDIUM (novel, risky) | HIGH | P2 (ship last, isolate blast radius) |
...
| Emoji reactions on tweets | No | No | No | N/A | Yes — real posted reply with undo (higher risk/value than the one client-side-only competitor found) |
